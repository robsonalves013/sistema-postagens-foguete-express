...
if opcao == "Dashboard":
    ...
    if not postagens:
        ...
        # Conversão robusta: aceita ambos formatos e evita problemas de agrupamento
        ...
        for ano, grupo_ano in df.groupby("ano"):
            with st.expander(f"📅 Ano: {ano}"):
                ...
                for mes in meses:
                    expand_mes = st.checkbox(
                        f"📅 Mês: {mes} - {len(grupo_ano[grupo_ano['mes'] == mes])} postagens",
                        key=f"{ano}_{mes}"
                    )
                    if expand_mes:
                        grupo_mes = grupo_ano[grupo_ano["mes"] == mes]
                        for _, p in grupo_mes.iterrows():
                            expand_post = st.checkbox(
                                f"📦 {p['codigo']} | {p['posto']} | {p['remetente']}",
                                key=f"post_{p['id']}"
                            )
                            if expand_post:
                                ...
                                st.write(f"**Data Postagem:** {p['data_postagem'].strftime('%d/%m/%Y %H:%M:%S')}")
                                ...
                                if admin:
                                    ...
                                    novo_posto = st.selectbox(
                                        "Posto", ["Shopping Bolivia", "Hotel Family"],
                                    ...
                                    nova_forma = st.selectbox(
                                        "Forma Pagamento", ["PIX", "Dinheiro", "Cartão"],
                                    ...
                                    # Inclusão: edição de data e hora de postagem
                                    data_dt = p['data_postagem'] if isinstance(p['data_postagem'], datetime) else pd.to_datetime(p['data_postagem'])
                                    nova_data = st.date_input("Data da Postagem", value=data_dt.date())
                                    nova_hora = st.time_input("Hora da Postagem", value=data_dt.time())
                                    ...
                                    if salvar:
                                        data_completa = datetime.combine(nova_data, nova_hora).strftime("%d/%m/%Y %H:%M:%S")
                                        novos_dados = (
                                            novo_posto, novo_remetente, novo_codigo, novo_tipo, novo_valor,
                                            nova_forma, novo_status, novo_func,
                                            data_completa, nova_data_pag, nova_observacao
                                        )
                                        try:
