...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://usuario:senha@host:porta/dbname")
    ...
    with conectar() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                (nome, usuario, hashed, int(is_admin))
            )
        conn.commit()
...
def autenticar(usuario, senha):
    with conectar() as conn:
        with conn.cursor() as cur:
            ...
            user = cur.fetchone()
            if user and bcrypt.checkpw(senha.encode("utf-8"), user['senha'].tobytes()):
                return user
    return None
...
def atualizar_pagamento(post_id, status, data_pagamento):
    with conectar() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE postagens SET status_pagamento=%s, data_pagamento=%s WHERE id=%s",
